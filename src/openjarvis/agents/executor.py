    ...
    def _finalize_tick(
        ...
        if error is None:
            ...
            if agent_data:
                ...
                # ``config["max_tokens"]`` is the per-run generation cap used
                # by agent templates (for example deep_research defaults to
                # 4096). Treating it as a cumulative lifetime budget flips
                # healthy agents into ``budget_exceeded`` after a few runs.
                # Use a dedicated budget key instead.
                max_total_tokens = config.get("max_total_tokens", 0) or config.get(
                    "token_budget", 0
                )
                exceeded = False
